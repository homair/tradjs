...
export function updateRoute (req, res) {
  ...
  if (config.flat_collection === true) {
    ...
    if (req.body._id) {
      ...
      logger.info('updateRoute: UPDATE, where=' + JSON.stringify(where) + ' value=' + JSON.stringify(req.body.value))
    } else {
      logger.info('updateRoute: INSERT, where=' + JSON.stringify(where) + ' value=' + JSON.stringify(req.body.value))
    }

    getDb().collection(config.db.root_collection).updateOne(where,
...
export function deleteRoute (req, res) {
  ...
  if (config.flat_collection === true) {
    for (let lang in config.langs) {
      ...
      getDb().collection(config.db.root_collection).deleteOne(where, (err, result) => {
        if (err) {
    ...
    for (let lang in config.langs) {
      getDb().collection(config.db.root_collection).updateOne({'language': lang}, {$unset: {[req.body.key]: ''}}, (err, result) => {
        if (err) {
