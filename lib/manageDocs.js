...
export function getOrderedDocs (langs, callback) {
  if (config.flat_collection === true) {
    getFlatOrderedDocs(langs, callback)
  } else {
    getRegularOrderedDocs(langs, callback)
  }
}

export function getRegularOrderedDocs (langs, callback) {
  ...
  getDb().collection(config.db.root_collection).find({'language': {$in: [...Object.keys(langs)]}}).toArray((err, docs) => {
    if (err) {
      callback(err, null)
    }
    // -----------------------------------------------------------------------------------------------
...
export function tri (a, b) {
  if (a.key < b.key) { return -1 } else if (a.nom === b.nom) { return 1 } else { return 1 }
}
...
export function updateRoute (req, res) {
  ...
  if (config.flat_collection === true) {
    getDb().collection(config.db.root_collection).updateOne({'language': req.body.language, 'namespace': config.translationNamespace, 'key': req.body.key},
                                                            { $set: {data: req.body.value} }, {upsert: true}, (err, result) => {
                                                              if (err) {
                                                                logger.error('updateRoute : err', err)
                                                                return res.status(500).send(err)
                                                              }
                                                              res.send('ok')
                                                            })
  } else {
    getDb().collection(config.db.root_collection).updateOne({'language': req.body.language}, {$set: {[req.body.key]: req.body.value}}, (err, result) => {
      if (err) {
        logger.error('updateRoute : err', err)
        return res.status(500).send(err)
      }
...
export function deleteRoute (req, res) {
  ...
  if (config.flat_collection === true) {
    for (let lang in config.langs) {
      getDb().collection(config.db.root_collection).deleteOne({'language': lang, 'namespace': config.translationNamespace, 'key': 'data.' + [req.body.key]}, (err, result) => {
        if (err) {
          logger.err('[deleteRoute-flat_collection] : err', err)
          return res.status(500).send(err)
        }
        finished++
    ...
    for (let lang in config.langs) {
      getDb().collection(config.db.root_collection).updateOne({'language': lang}, {$unset: {['data.' + req.body.key]: ''}}, (err, result) => {
        if (err) {
          logger.err('[deleteRoute-regular_collection] : err', err)
          return res.status(500).send(err)
        }
        finished++
        if (finished === Object.keys(config.langs).length) {
...
export function getFlatOrderedDocs (langs, callback) {
  logger.debug('getFlatOrderedDocs')

  const hrstart = process.hrtime()

  /* ------ Recupère l'ensemble des documents de la collection "i18next" en fonction de la langue défini dans la config. -------- */
  getDb().collection(config.db.root_collection).aggregate([
    ...
    { $project: { key: { $substr: [ '$_id', 5, -1 ] }, values: 1, _id: 0 } }
  ]).toArray((err, docs) => {
    if (err) {
      callback(err, null)
    }
    ...
    docs.forEach((el) => {
      ...
      config.lang_order.forEach((lang) => {
        if (oValues[lang] && oValues[lang].value) {
          curr[lang] = oValues[lang].value
        }
      })
      objReturn.push(curr)
    })
