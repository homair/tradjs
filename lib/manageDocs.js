...
export function getOrderedDocs (langs, callback) {
  ...
  getDb().collection(config.db.root_collection + '_' + config.env).find({'language': {$in: [...Object.keys(langs)]}}).toArray((err, docs) => {
    if (err) {
      callback(err, null)
    }
    // -----------------------------------------------------------------------------------------------
...
export function updateRoute (req, res) {
  getDb().collection(config.db.root_collection + '_' + config.env).updateOne({'language': req.body.language}, {$set: {[req.body.key]: req.body.value}}, (err, result) => {
    if (err) {
...
export function deleteRoute (req, res) {
  ...
  for (let lang in config.langs) {
    getDb().collection(config.db.root_collection + '_' + config.env).updateOne({'language': lang}, {$unset: {['data.' + req.body.key]: ''}}, (err, result) => {
      if (err) {
